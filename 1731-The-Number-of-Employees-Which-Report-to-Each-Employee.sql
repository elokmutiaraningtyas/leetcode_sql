...
SELECT b.employee_id, a.name, b.reports_count, b.average_age
FROM
    (SELECT distinct reports_to as employee_id, 
            count(name) as reports_count, 
            round(avg(age),0) as average_age
    FROM Employees
    where reports_to is not null -- filter to get only person who have employee reporting to them as the manager
    group by 1) b, 
